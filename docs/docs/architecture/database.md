...
Asclepius uses SQLite with WAL (Write-Ahead Logging) mode and FTS5 (Full-Text Search) for all structured data storage. The database file is stored at `vault/asclepius.sqlite`.
