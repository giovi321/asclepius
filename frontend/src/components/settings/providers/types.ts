...
export const CREDENTIAL_TYPES = [
  ...
  },
  {
    value: "vllm",
    label: "vLLM",
    description: "vLLM (OpenAI-compatible)",
    icon: Server,
    needs_url: true,
    needs_key: true,
  },
  ...
  {
    value: "tesseract_remote",
    label: "Tesseract (Remote)",
    description: "Remote Tesseract OCR server",
    icon: Server,
    needs_url: true,
    needs_key: false,
  },
...
export function allowedKindsFor(credType: string): ModelKind[] {
  if (credType === "google_vision" || credType === "tesseract_remote")
    return ["ocr"];
