...
import { COLUMNS as DOC_COLUMNS, DOCUMENTS_DEFAULTS } from "@/components/documents/columns";
 ...
 * rather than fixed columns and is excluded for now (tracked for 0.9.13).
 *
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Pick which columns appear in each list view, and reorder them. Saves automatically.
        </p>
      </div>
      ...
      <ViewSection
        title="Documents"
        viewKey="documents"
        defs={DOC_COLUMNS.map((c) => ({ key: c.key as string, label: c.label }))}
      ...
      <ViewSection
        title="Imaging"
        viewKey="imaging"
        defs={IMAGING_COLUMNS.map((c) => ({ key: c.key as string, label: c.label }))}
...
function ViewSection({ title, viewKey, defs, defaults }: {
  title: string;
          ...
          return (
            <li key={k} className="flex items-center gap-2 px-3 py-2 text-sm">
              <input
                ...
                className="h-4 w-4"
              />
              <span className={`flex-1 ${visible ? "" : "text-muted-foreground/60"}`}>
                {def.label}
              </span>
              <button
                onClick={() => move(k, "up")}
