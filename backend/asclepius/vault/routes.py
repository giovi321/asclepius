...
async def delete_vault_file(
    ...
    if file_path.is_dir():
        raise HTTPException(
            status_code=400, detail="Cannot delete directories. Delete files individually."
        )
...
async def move_vault_file(
    ...
    if path_filter is not None:
        ...
        if not path_filter(src_rel, is_dir) or not path_filter(dst_rel, is_dir):
            raise HTTPException(
                status_code=403, detail="You don't have access to one of these paths"
            )
    ...
    await db.execute(
        "UPDATE documents SET file_path = ?, updated_at = CURRENT_TIMESTAMP " "WHERE file_path = ?",
        (dst_rel, src_rel),
    )
    await db.execute(
