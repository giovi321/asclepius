...
class AliasCache:
    ...
    async def ensure_loaded(
        self, db: aiosqlite.Connection, entity: str,
    ) -> None:
    ...
    *,
    ...
    if aliases:
        try:
            ...
            if best is not None:
                ...
                await _add_alias(db, entity, norm_id, cleaned,
                                 auto_mapped=True)
                cache.remember(entity, cleaned, norm_id)
                logger.debug(
                    "Resolver: fuzzy match %s → alias %r score=%.1f",
                    entity, cleaned, score,
                )
                return norm_id
        except Exception:
            logger.debug("rapidfuzz lookup failed for %s=%r", entity, cleaned,
                         exc_info=True)
    ...
    if tables.extra_cols:
        ...
        params = [canonical_code, cleaned,
                  *(extra.get(col) for col in tables.extra_cols)]
    else:
    ...
    logger.debug(
        ...
        entity, norm_id, canonical_code, cleaned,
    )
    return norm_id
...
async def resolve_lab_test(
    db: aiosqlite.Connection, cache: AliasCache, original: str,
) -> int | None:
...
async def resolve_extraction(
    db: aiosqlite.Connection, extraction: dict,
) -> dict:
    ...
    for diag in extraction.get("diagnoses") or []:
        ...
        norm_id = await resolve_diagnosis(
            db, cache, original, icd10_code=diag.get("icd10_code"),
        )
        if norm_id is not None:
