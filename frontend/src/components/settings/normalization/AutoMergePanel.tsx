...
export default function AutoMergePanel({
  proposals, entries,
  onClose, onApply, onSkip, onUpdateTarget, onToggleSource,
}: AutoMergePanelProps) {
  const entryById: Record<number, any> = Object.fromEntries(entries.map((e: any) => [e.id, e]));
  ...
  return (
    <div className="rounded-lg border border-primary/30 bg-primary/5 p-4 space-y-3">
      ...
      {proposals.length === 0 ? (
        <p className="text-sm text-muted-foreground">No merge candidates found. All entries look distinct.</p>
      ) : (
            ...
            return (
              <div key={idx} className="rounded-md border bg-background p-3 space-y-2">
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  ...
                  <select
                    value={p.target_id}
                    onChange={(e) => onUpdateTarget(idx, Number(e.target.value))}
                  ...
                  {p.source_ids.length === 0 ? (
                    ...
                    p.source_ids.map((sid: number) => (
                      <label key={sid} className="flex items-center gap-2 text-sm">
                        <input type="checkbox" checked onChange={() => onToggleSource(idx, sid)} />
