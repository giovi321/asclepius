 ...
 *
 * Migrates legacy localStorage choices on first load so existing users
 * don't lose their setup when they upgrade to 0.9.12.
 */
...
export function useColumnPrefs(
  ...
  // Initial load — prefer backend, fall back to localStorage (legacy
  // documents page had a localStorage entry pre-0.9.12), fall back to
  // hard-coded defaults.
  useEffect(() => {
    let cancelled = false;
    (async () => {
            ...
            if (Array.isArray(arr) && arr.length > 0) {
              ...
              try {
                await api.put(`/settings/view-prefs/${viewKey}`, {
                  visible: arr, order: defaults.order,
                });
  ...
  const persist = useCallback(async (v: string[], o: string[]) => {
    try {
      await api.put(`/settings/view-prefs/${viewKey}`, { visible: v, order: o });
