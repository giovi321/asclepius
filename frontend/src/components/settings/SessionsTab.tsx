...
function describeUserAgent(ua: string | null): { label: string; icon: typeof Monitor } {
  ...
  const browser =
    ...
    /curl|python|httpx|axios/.test(s) ? "API client" :
    "Browser";
  const os =
...
export default function SessionsTab() {
  ...
  const counts = useMemo(() => {
    ...
    return { total: sessions.length, active, revoked: sessions.length - active };
  }, [sessions]);
  ...
  const revoke = async (row: SessionRow) => {
    ...
    try {
      await api.delete(`/settings/sessions/${encodeURIComponent(row.session_id)}`);
      if (row.is_current) {
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            {counts.active} active{includeRevoked ? ` • ${counts.revoked} revoked/expired` : ""}
          </p>
        </div>
        ...
        <div className="overflow-hidden rounded-lg border">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y">
              {filtered.map((s) => {
                const { label: uaLabel, icon: UAIcon } = describeUserAgent(s.user_agent);
                const isRevoked = s.revoked_at !== null;
                const isExpired = !isRevoked && new Date(s.expires_at + "Z").getTime() < Date.now();
                return (
                  <tr key={s.session_id} className={isRevoked || isExpired ? "opacity-60" : s.is_current ? "bg-primary/5" : ""}>
                    <td className="px-3 py-1.5">
                      <div className="font-medium">{s.display_name || s.username}</div>
                      <div className="text-xs text-muted-foreground">@{s.username} • {s.role}</div>
                    </td>
                    <td className="px-3 py-1.5">
                      <span className="inline-flex items-center gap-1.5" title={s.user_agent || ""}>
                        <UAIcon className="h-3.5 w-3.5 text-muted-foreground" />
                        <span className="truncate max-w-[180px]">{uaLabel}</span>
                      </span>
                    </td>
                    ...
                    <td className="px-3 py-1.5">
                      {isRevoked ? (
                        <span className="rounded-full bg-gray-100 px-2 py-0.5 text-xs text-gray-700 dark:bg-gray-900/30 dark:text-gray-400">revoked</span>
                      ) : isExpired ? (
                        <span className="rounded-full bg-amber-100 px-2 py-0.5 text-xs text-amber-700 dark:bg-amber-900/30 dark:text-amber-400">expired</span>
                      ) : s.is_current ? (
