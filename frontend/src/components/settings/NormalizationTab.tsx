...
import AutoMergePanel, {
  type AutoMergeProposal,
} from "./normalization/AutoMergePanel";
    ...
    <div className="space-y-4">
      ...
      {linkedDocsFor && (
        <LinkedDocumentsModal
          ...
          onDelete={() =>
            linkedDocsFor && handleDelete(linkedDocsFor.id, linkedDocsFor.name)
          }
        />
      )}
            ...
            {normItems.length === 0 ? (
              ...
              normItems.map((item) => (
                <NormalizationRow
                  ...
                  onToggleMerge={() =>
                    setShowMergeFor(showMergeFor === item.id ? null : item.id)
                  }
                  onCancelMerge={() => {
                    setShowMergeFor(null);
                    setMergeTargetId(null);
                  ...
                  onCancelEdit={() => {
                    setEditing(false);
                    setSaveError(null);
                    setEditCode(detail?.canonical_code || "");
                    setEditDisplay(detail?.canonical_display || "");
                  }}
                  ...
                  onMerge={() =>
                    mergeTargetId && handleMerge(item.id, mergeTargetId)
                  }
                />
              ))
            )}
