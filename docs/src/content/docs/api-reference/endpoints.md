...
| Method | Path | Auth | Description |
|--------|------|------|-------------|
| `GET` | `/api/share/{token}/info` | Token | Returns `{delivery: "manual" \| "email", to_masked: string \| null}` so the doctor's landing/verify pages can show the right copy ("Check the inbox at j***@example.com" vs "Ask your contact for the code"). For invalid or revoked tokens this returns the same shape as a valid manual share (`{"delivery": "manual", "to_masked": null}`), so the only thing the endpoint reveals about token validity is whether it resolves to an *email* share. |
