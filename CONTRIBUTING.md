...
- **Do not open public issues for security problems.** Open a private advisory
  from the repo's [Security tab](https://github.com/giovi321/asclepius/security)
  instead.
...
- `pre-commit` (optional but recommended; `pre-commit install` after the
  backend dev install wires up `ruff`, `prettier`, and the OpenAPI / schema
  drift hook so failures land at commit time instead of in CI)
...
Copy [`.env.example`](.env.example) to `.env` and set `SECRET_KEY` to a
random string (the backend will refuse to start with the placeholder in
production mode):
...
3. Document any threat-model change or new configuration knob in the relevant
   docs page under `docs/src/content/docs/admin-guide/`.
