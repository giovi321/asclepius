 ...
 */
export function looksLikeOgtt(
  rows: Array<{ test_name_original?: string | null }>,
): boolean {
  const parsed = rows.filter(
    (r) => parseOgttMinutes(r.test_name_original) !== null,
  );
