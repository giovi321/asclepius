...
import {
  Check, ChevronRight, Edit3, FileText, GitMerge, Plus, Save, Trash2,
} from "lucide-react";
...
export default function NormalizationRow(props: NormalizationRowProps) {
  const {
    item, normItems, detail, expanded, editing,
    editCode, editDisplay, saveError, newAlias, newAliasLang,
    showMerge, mergeTargetId, rowNewDisplay, rowNewCode,
    selected, onToggleSelect, onToggleExpand,
    onViewDocuments, onToggleMerge, onCancelMerge, onDelete,
    onStartEdit, onCancelEdit, onEditCodeChange, onEditDisplayChange, onSaveEdit,
    onNewAliasChange, onNewAliasLangChange, onAddAlias, onDeleteAlias, onConfirmAll,
    onMergeTargetChange, onRowNewDisplayChange, onRowNewCodeChange, onMerge,
  } = props;

  return (
    <React.Fragment>
      <tr className={`cursor-pointer transition-colors ${expanded ? "bg-accent/30" : "hover:bg-accent/20"}`}
        ...
        <td className="px-3 py-1.5 font-mono text-xs text-muted-foreground max-w-[280px] truncate" title={item.canonical_code || undefined}>{item.canonical_code}</td>
        ...
        <td className="px-3 py-1.5 whitespace-nowrap w-px" onClick={(e) => e.stopPropagation()}>
      ...
      {showMerge && (
        <tr className="bg-orange-50 dark:bg-orange-900/10">
          <td colSpan={6} className="px-6 py-3" onClick={(e) => e.stopPropagation()}>
            <div className="flex flex-col gap-2 text-sm">
              <div className="flex flex-wrap items-center gap-3">
                ...
                <button onClick={onCancelMerge} className="rounded-md border px-2 py-1 text-xs hover:bg-accent">
                  Cancel
                </button>
              </div>
              {mergeTargetId === -1 && (
                  ...
                  <div className="grid gap-2">
                    <div className="grid grid-cols-2 gap-2">
                      <label className="space-y-1">
                        ...
                        <input type="text" value={editCode} onChange={(e) => onEditCodeChange(e.target.value)}
                          className="w-full rounded-md border bg-background px-2.5 py-1.5 text-sm font-mono" />
                      </label>
                      <label className="space-y-1">
                        <span className="text-xs text-muted-foreground">Display Name</span>
                        <input type="text" value={editDisplay} onChange={(e) => onEditDisplayChange(e.target.value)}
              ...
              <div className="space-y-2">
                  ...
                  <div className="rounded-md border divide-y max-h-[300px] overflow-y-auto">
                    {detail.aliases.map((a: any) => (
                      <div key={a.id} className="flex items-center justify-between px-3 py-1.5 text-sm hover:bg-accent/20">
                        ...
                        <button onClick={() => onDeleteAlias(a.id)}
                ...
                <div className="flex gap-2 items-end">
                  <label className="space-y-1 flex-1">
                    ...
                    <input type="text" value={newAlias}
                  ...
                  <label className="space-y-1 w-20">
                    ...
                    <input type="text" value={newAliasLang}
                      onChange={(e) => onNewAliasLangChange(e.target.value)}
                      placeholder="en"
                      onKeyDown={(e) => e.key === "Enter" && onAddAlias()}
                      className="w-full rounded-md border bg-background px-2.5 py-1.5 text-sm" />
                  </label>
                  <button onClick={onAddAlias} disabled={!newAlias.trim()}
