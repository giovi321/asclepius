...
export default function BatchMergeBar({
  selectedCount, normItems, selectedIds,
  batchTargetId, onBatchTargetChange,
  batchNewDisplay, onBatchNewDisplayChange,
  batchNewCode, onBatchNewCodeChange,
  onMerge, onClear,
}: BatchMergeBarProps) {
  ...
  return (
    <div className="flex flex-col gap-2 rounded-md border border-orange-300 dark:border-orange-700 bg-orange-50 dark:bg-orange-900/20 px-3 py-2 text-sm">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <button onClick={onClear} className="rounded-md border px-2 py-1 text-xs hover:bg-accent">Clear</button>
      </div>
      {batchTargetId === -1 && (
