...
export default function OgttCurveChart({ rows }: { rows: LabRow[] }) {
  ...
  for (const p of points) {
    ...
    merged.set(p.minutes, existing == null ? p.value : (existing + p.value) / 2);
  }
        ...
        <ResponsiveContainer>
          <LineChart data={data} margin={{ top: 5, right: 16, left: 0, bottom: 5 }}>
            ...
            {unit.toLowerCase().includes("mg") && (
              <>
                <ReferenceLine y={140} stroke="#f59e0b" strokeDasharray="4 3" label={{ value: "IGT 140", position: "right", fontSize: 10, fill: "#f59e0b" }} />
                <ReferenceLine y={200} stroke="#ef4444" strokeDasharray="4 3" label={{ value: "Diabetes 200", position: "right", fontSize: 10, fill: "#ef4444" }} />
              </>
            )}
            <Line
              type="monotone"
