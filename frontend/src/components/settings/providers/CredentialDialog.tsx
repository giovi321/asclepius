...
export default function CredentialDialog({ initial, onSave, onClose }: CredentialDialogProps) {
  ...
  const handleSave = async () => {
    ...
    if (backoff.length === 0) {
      setErr("Retry backoff must be a comma-separated list of non-negative integers");
      return;
    }
    setSaving(true);
    try {
