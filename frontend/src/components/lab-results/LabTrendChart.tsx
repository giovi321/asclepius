...
export default function LabTrendChart({ rows, selectedCodes }: Props) {
  ...
  for (const r of usable) {
    ...
    byDate.get(key)![code] = existing == null ? (r.value as number) : (existing + (r.value as number)) / 2;
  }
  ...
  const ranges = usable.filter(
    (r) => r.canonical_code === first && r.reference_range_low != null && r.reference_range_high != null,
  );
  if (ranges.length > 0) {
    const counts = new Map<string, { n: number; low: number; high: number }>();
    for (const r of ranges) {
      const k = `${r.reference_range_low}-${r.reference_range_high}`;
      const e = counts.get(k) || { n: 0, low: r.reference_range_low!, high: r.reference_range_high! };
      e.n += 1;
      counts.set(k, e);
    }
  ...
  return (
    <div className="rounded-lg border bg-background p-3">
      <div className="mb-2 flex items-center justify-between text-xs text-muted-foreground">
        <span>Trend — {data.length} data point{data.length === 1 ? "" : "s"}</span>
        {refBand && (
          <span>Reference band (shaded): {refBand.low}–{refBand.high}</span>
        )}
      </div>
