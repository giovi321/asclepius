...
| `imaging_studies` | Imaging study metadata (modality, body part, accession, study UID, ``report_status``). Doctor + facility are foreign keys kept in lockstep with the parent ``documents`` row via the ``imaging_studies_doctor_sync`` / ``_facility_sync`` AFTER UPDATE triggers, do not write them here directly. |
