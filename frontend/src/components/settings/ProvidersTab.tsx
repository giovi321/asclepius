...
export default function ProvidersTab() {
  ...
  const vision: VisionLlmProvider[] = Array.isArray(visionData)
    ? visionData
    : [];
  const ocr: OcrProvider[] = Array.isArray(ocrData) ? ocrData : [];
  const [editingCred, setEditingCred] = useState<Partial<Credential> | null>(
    null,
  );
  ...
  const attachedByCred = useMemo(() => {
    ...
    for (const p of ocr) {
      ...
      const label =
        p.type === "llm_vision"
          ? p.name || p.llm_model || "LLM vision OCR"
          : p.type === "google_vision"
            ? p.name || "Google Vision OCR"
            : p.type === "tesseract_remote"
              ? p.name || "Tesseract remote"
              : p.name || p.type;
      arr.push({
        kind: "ocr",
        entry_id: p.id,
  ...
  const handleDeleteCredential = async (id: string) => {
    ...
    if ((cred.references?.total || 0) > 0) {
      toast({
        title: `"${cred.name}" has attached models; remove them first`,
        variant: "error",
      });
      return;
    }
  ...
  const addModel = async (
    ...
    if (kind === "llm") {
      ...
      const ocrType =
        cred.type === "google_vision"
          ? "google_vision"
          : cred.type === "tesseract_remote"
            ? "tesseract_remote"
            : "llm_vision";
  ...
  const removeModel = async (m: AttachedModel) => {
    if (m.kind === "llm") {
      await api.put(
        ...
        llm.filter((p) => p.id !== m.entry_id),
      );
    } else if (m.kind === "vision") {
      await api.put(
        "/settings/vision-providers",
        vision.filter((p) => p.id !== m.entry_id),
      );
    } else {
      await api.put(
        "/settings/ocr-providers",
        ocr.filter((p) => p.id !== m.entry_id),
      );
    }
    await reloadAll();
  };
  ...
  const testModel = async (
    ...
    const path =
      m.kind === "vision"
        ? "/settings/test-vision-provider"
        : m.kind === "ocr"
          ? "/settings/test-ocr-provider"
          : "/settings/test-llm-provider";
    try {
      const resp = await api.post(path, { provider_id: m.entry_id });
      const data = resp.data ?? {};
      if (data.ok) {
        return { ok: true, message: data.detail || "Connection OK" };
      }
      return { ok: false, message: data.error || "Test failed" };
    } catch (e: any) {
      const message =
  ...
  const toggleModel = async (m: AttachedModel) => {
    if (m.kind === "llm") {
      await api.put(
        "/settings/llm-providers",
        llm.map((p) =>
          p.id === m.entry_id ? { ...p, enabled: !p.enabled } : p,
        ),
      );
    } else if (m.kind === "vision") {
      await api.put(
  ...
  return (
    <div className="space-y-4">
      <div className="text-sm text-muted-foreground">
        ...
        retry policy). Under each provider, list the models it exposes - LLM,
        Vision, or OCR. Ranking and task assignment happens on the
        ...
        {credentials.map((c) => (
          <CredentialCard
            ...
            onAddModel={(kind, name, model, timeout) =>
              addModel(c, kind, name, model, timeout)
            }
            onStartEditModel={(key) => {
