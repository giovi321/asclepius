...
export default function LinkedDocumentsModal({
  subjectName, loading, documents, onClose, onDelete,
}: LinkedDocumentsModalProps) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
      <div className="max-h-[80vh] w-full max-w-2xl overflow-y-auto rounded-lg border bg-background p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
        <div className="mb-3 flex items-center justify-between">
          <h3 className="text-base font-semibold">Documents referencing "{subjectName}"</h3>
        ...
        {loading ? (
          ...
          <div className="space-y-3 py-4">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={onClose} className="rounded-md border px-3 py-1.5 text-xs hover:bg-accent">Cancel</button>
            </div>
          </div>
        ) : (
