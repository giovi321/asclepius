...
The frontend types in `src/api/schema.ts` are generated from the FastAPI OpenAPI spec. Run both steps after touching a request/response model on the backend:
