...
_EXAMPLE_KEYS = {
    ...
    "doc_date",
    "date_issued",
    "date_visit",  # legacy keys from older extractions
    "language_detected",
    "doctor",
    "facility",
    "specialty",
...
def _compact_extraction(raw: dict) -> dict:
    ...
    for key in _EXAMPLE_KEYS:
        ...
        if isinstance(val, dict):
            compact = {
                k: v for k, v in val.items() if v and k in ("name", "type", "original", "canonical")
            }
            if compact:
                result[key] = compact
        else:
...
async def find_few_shot_examples(
    ...
    if facility_id and len(examples) < limit:
        ...
        for row in await cursor.fetchall():
            ...
            corrections = [
                {"field_name": r[0], "corrected_value": r[1]} for r in await corr_cursor.fetchall()
            ]
    ...
    if len(examples) < limit:
        ...
        if terms:
            ...
            try:
                exclude_clause = (
                    "AND d.id NOT IN ({})".format(",".join("?" * len(seen_ids))) if seen_ids else ""
                )
                cursor = await db.execute(
    ...
    logger.info(
        "Found %d few-shot examples for extraction (facility_id=%s)", len(examples), facility_id
    )
    return examples
