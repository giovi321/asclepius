...
async def _row_with_doc(
    db: aiosqlite.Connection, table: str, row_id: int,
) -> tuple[dict, dict]:
...
_ENCOUNTER_TEXT_FIELDS = {
    "diagnosis_original", "diagnosis_code", "notes", "findings",
}
    ...
    for field in body.model_fields_set:
        ...
        if field in _ENCOUNTER_TEXT_FIELDS:
            ...
            if field == "diagnosis_original":
                norm_id = (
                    await resolve_diagnosis(db, cache, new_val)
            ...
            norm_id = (
                await resolve_specialty(db, cache, new_val)
                if new_val else None
            )
    ...
    for field, old_val, new_val in corrections:
        await db.execute(
            """INSERT INTO extraction_corrections
               ...
               VALUES (?, ?, ?, ?, ?)""",
            (rec["document_id"], f"encounter.{field}", old_val, new_val,
             doc.get("doc_type") or "encounter"),
        )
    await db.commit()

    await audit_log(
        db, current_user["id"], "encounter.update", "encounter", encounter_id,
...
_MEDICATION_TEXT_FIELDS = {
    "active_ingredient_original", "brand_name", "dosage", "form",
...
async def update_medication(
    ...
    for field in body.model_fields_set:
        ...
        if field == "active_ingredient_original":
            norm_id = (
                await resolve_medication(db, cache, new_val)
                if new_val else None
            )
    ...
    for field, old_val, new_val in corrections:
        await db.execute(
            """INSERT INTO extraction_corrections
               (document_id, field_name, llm_value, corrected_value, doc_type)
               VALUES (?, ?, ?, ?, ?)""",
            (rec["document_id"], f"medication.{field}", old_val, new_val,
             doc.get("doc_type") or "prescription"),
        )
    await db.commit()

    await audit_log(
        db, current_user["id"], "medication.update", "medication", medication_id,
        {"fields": [c[0] for c in corrections]}, get_client_ip(request),
    )
