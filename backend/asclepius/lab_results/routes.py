...
async def _get_lab_result(db: aiosqlite.Connection, result_id: int) -> dict | None:
    cursor = await db.execute("SELECT * FROM lab_results WHERE id = ?", (result_id,))
...
async def list_lab_results(
    ...
    if test_name:
        conditions.append("(lr.test_name_original LIKE ? OR nlt.canonical_display LIKE ?)")
...
async def create_lab_result(
    ...
    """
    cursor = await db.execute("SELECT patient_id FROM documents WHERE id = ?", (body.document_id,))
...
async def update_lab_result(
    ...
    try:
        ...
        await db.execute(f"UPDATE lab_results SET {set_clause} WHERE id = ?", values)
