...
export default function NormalizationToolbar({
  normType, onNormTypeChange,
  normFilter, onNormFilterChange,
  searchInput, onSearchInputChange, onSearchCommit,
  onAutoMerge, autoMergeLoading, canAutoMerge,
  itemCount,
  ...
  return (
    <div className="flex flex-wrap gap-3 items-center">
      <select value={normType} onChange={(e) => onNormTypeChange(e.target.value)}
      ...
      <select value={normFilter || ""} onChange={(e) => onNormFilterChange(e.target.value || null)}
        className="rounded-md border bg-background px-3 py-2 text-sm">
      ...
      <div className="relative flex-1 min-w-[200px] max-w-sm">
        ...
        {searchInput && (
          <button onClick={() => { onSearchInputChange(""); onSearchCommit(""); }}
