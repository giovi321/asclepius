...
_USER_KNOWLEDGE_DIR = (
    Path(os.environ.get("ASCLEPIUS_CONFIG_PATH", "/data/config/settings.yaml")).parent / "knowledge"
)
...
_BUNDLED_KNOWLEDGE_DIR = Path(__file__).parent.parent.parent / "bundled_config" / "knowledge"
...
_REPO_KNOWLEDGE_DIR = Path(__file__).parent.parent.parent.parent / "bundled_config" / "knowledge"
    ...
    def _load(self) -> None:
        ...
        for entry in data:
            ...
            code = (
                ...
                or entry.get("atc_code")
                or entry.get("loinc_code")
                or entry.get("icd10_code")
            )
            if not code:
                continue
            self._entry_count += 1
        ...
        logger.info(
            "knowledge_base[%s] loaded from %s: %d entries, %d unique aliases" "%s",
            self.kind,
            path,
            self._entry_count,
            len(self._lookup),
