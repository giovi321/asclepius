...
async def process_dicom(
    ...
    frame_already_ingested = dest.exists() and dest.stat().st_size == frame_size
    ...
    # The parent document represents the radiology REPORT (PDF). For zip
    # uploads that arrive without a report attached, the document is a
    # placeholder: file_path stays NULL until the user uploads the PDF.
    # The first DICOM frame creates the placeholder row; every subsequent
    # frame just looks it up by the deterministic study hash.
    cursor = await db.execute(
    ...
    if row:
        ...
        await db.execute(
            """INSERT INTO imaging_series
               ...
               VALUES (?, ?, ?, ?, 1, ?, ?)""",
            (study_id, series_number, effective_desc, modality, series_uid, series_folder_path),
        )
